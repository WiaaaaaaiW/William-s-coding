...
class GameInterface:
    # Displays start screen of the battle with player and enemy names
    def display_start_screen(self, player, enemy):
        print(f"\n Start Battle: {player.name} vs {enemy.name}")

    # Displays a message indiating the start of a new turn
    def display_next_turns(self):
        print(f"\n Next Turn")

    # Displays current status (HP) of both characters
    def display_status(self, player, enemy):
        player.display_status()
        enemy.display_status()

    # Shows the available skills of a character
    def display_skills(self, character):
        character.display_skills()

    # Prompts the player to choose a skill by number selection
    def display_skill_choice(self, character):
        while True:
            try:
                selection = int(input("\nChoose a skill (number): ")) - 1
                if 0 <= selection < len(character.skills):
                    return character.skills[selection]
                else:
                    print("Invalid not found. Try again.")
            except ValueError:
                print("Please enter a number.")

    # Shows the result of a skill being used 
    def display_battle_result(self, user, skill, damage, ):
        print(f"{user.name} used {skill.name} causing {damage} damage!")
    
    # Display a message when a character is defeated
    def display_defeat(self, character):
        print(f"\n{character.name} has been defeated")
    

    # Displays a message when it's the enemy's turn
    def display_enemy_turn(self, enemy):
        print(f"\n{enemy.name}'s turn!")

    # Displays a message when its the player's turn
    def display_player_turn(self, player):
        print(f"\n{player.name}'s turn!")

    # Display a declaration message on who win at the end of battle
